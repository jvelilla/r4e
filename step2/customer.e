...
feature -- Operations
	...
	statement: STRING_32
		local
			...
			item: RENTAL
		do
			Result := {STRING_32}"Rental record for " + name + "%N"
			across rentals as ic
			loop
				item := ic.item

					-- determine amounts for each  line
					-- extract method amount_for.
					-- IDE EiffelStudio does not support extract method.
					-- Select the code and offer extract method option.
				this_amount := amount_for (item)
				...
				Result.append ({STRING_32}"%T" +item.movie.title + "%T" + this_amount.out + "%N")
...
feature {NONE} -- Implementation
	...
	amount_for (a_rental: RENTAL): REAL_64
			-- amount for the current `a_rental`.
		do
			Result := 0
			inspect a_rental.movie.price_code
			when {MOVIE}.REGULAR then
				Result := Result + 2
				if a_rental.days_rented > 2 then
					Result := Result + (a_rental.days_rented - 2) * 1.5
				end
			when {MOVIE}.NEW_RELEASE then
				Result := Result + a_rental.days_rented * 3
			when {MOVIE}.CHILDRENS then
				Result := Result + 1.5
				if a_rental.days_rented > 3 then
					Result := Result + (a_rental.days_rented - 3) * 1.5
				end
			end
		end
end
